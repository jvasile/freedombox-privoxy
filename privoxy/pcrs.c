...
/*********************************************************************
 *
 * Function    :  pcrs_execute_multiple_command
 *
 * Description :  Apply multiple pcrs commands to the subject.
 *                The subject is one or more commands separated by tab characters.
 *                The subject itself is left untouched, memory for the result
 *                is malloc()ed and it is the caller's responsibility to free
 *                the result when it's no longer needed.
 *
 ...
 *
 *********************************************************************/
char *pcrs_execute_multiple_command(const char *subject, const char *pcrs_command, int *hits)
{
   char * pch;
   pch = strtok (pcrs_command,"\t");
	...
	{
	  printf("%s", pch);
 	   subject = pcrs_execute_single_command(subject, pch, hits);
 	   pch = strtok (NULL, "\t");
	}
	return subject;
}
